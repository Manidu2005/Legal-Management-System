<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="heading-section !text-2xl">{{ __('Add Case Category') }}</h2>
        </div>
    </x-slot>

    <div class="max-w-2xl mx-auto">
        <div class="glass-card overflow-hidden">
            <form method="POST" action="{{ route('case-categories.store') }}" class="p-6 sm:p-8"
                  x-data="{ level: '{{ old('level', $preselectedLevel ?? '') }}' }">
                @csrf
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <select id="level" name="level" x-model="level" class="input-dynamic mt-1" required>
                            ...
                            <option value="1" @selected(old('level') == '1')>{{ __('Main Type (top level, e.g. Civil)') }}</option>
                            <option value="2" @selected(old('level') == '2')>{{ __('Group (under a Main Type, e.g. Property & Land Law)') }}</option>
                            <option value="3" @selected(old('level') == '3')>{{ __('Specific Type (leaf — what a case is filed under)') }}</option>
                        </select>
                    ...
                    <div x-show="level === '2'" x-cloak>
                        ...
                        <select id="parent_id_main" name="parent_id" class="input-dynamic mt-1" :disabled="level !== '2'">
                            ...
                            @foreach($mainTypes as $mainType)
                                <option value="{{ $mainType->id }}" @selected(old('parent_id', $preselectedParentId ?? null) == $mainType->id)>{{ $mainType->name }}</option>
                            @endforeach
                        </select>
                    </div>
                            ...
                            @foreach($groups->groupBy(fn($g) => $g->parent->name ?? '—') as $mainName => $groupOptions)
                                <optgroup label="{{ $mainName }}">
                                    @foreach($groupOptions as $groupOption)
                                        <option value="{{ $groupOption->id }}" @selected(old('parent_id', $preselectedParentId ?? null) == $groupOption->id)>{{ $groupOption->name }}</option>
                                    @endforeach
                                </optgroup>
                            @endforeach
                        </select>
