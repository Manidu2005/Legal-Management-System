<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between w-full">
            <h2 class="heading-display !text-3xl text-slate-800">{{ __('Document Vault') }}</h2>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($documents as $document)
                        <tr class="table-row-dynamic group">
                            <td>
                                <div class="font-medium text-slate-800 truncate max-w-[240px]">
                            ...
                            <td>
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-slate-600 text-sm">{{ $document->uploader->name ?? __('Unknown') }}</span>
                                </div>
                            </td>
                            <td>
                            ...
                            <td class="text-right">
                                <div class="flex items-center justify-end gap-2 transition-opacity duration-200">
                                    <a href="{{ route('documents.show', $document) }}" class="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="{{ __('View Details') }}">
                                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                        </svg>
                                    </a>
                                    <a href="{{ Storage::disk('public')->url($document->file_path) }}" download class="p-1.5 text-slate-400 hover:text-sky-600 hover:bg-sky-50 rounded-lg transition-colors" title="{{ __('Download') }}">
                                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                ...
                                <h3 class="text-lg font-medium text-slate-900">{{ __('No documents found') }}</h3>
                                <p class="mt-1 text-slate-500">{{ __('Upload your first document to get started.') }}</p>
                                <div class="mt-6">
                                    <a href="{{ route('documents.create') }}" class="btn-primary">
                                        {{ __('Upload Document') }}
                                    </a>
